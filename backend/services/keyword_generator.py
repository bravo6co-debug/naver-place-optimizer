...
class KeywordGeneratorService:
    ...
    async def generate_keywords(
        ...
        if not keywords and cat_data:
            keywords = self._generate_fallback_keywords(category, location, cat_data)
        elif not keywords:
            # 커스텀 업종이고 GPT도 실패한 경우 - 기본 키워드 생성
            keywords = self._generate_generic_keywords(category, location)
    ...
    def _generate_generic_keywords(self, category: str, location: str) -> List[Dict]:
        """
        커스텀 업종용 기본 키워드 생성
